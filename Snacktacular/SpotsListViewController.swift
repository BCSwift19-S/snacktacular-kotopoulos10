...
extension SpotsListViewController: FUIAuthDelegate {
    ...
    func authUI(_ authUI: FUIAuth, didSignInWith user: User?, error: Error?) {
        if let user = user {
            tableView.isHidden = false
            print("User signed in with name \(user.displayName ?? "No name in optional")")
        }
    }
    ...
    func authPickerViewController(forAuthUI authUI: FUIAuth) -> FUIAuthPickerViewController {
        ...
        let marginsInset: CGFloat = 16
        let imageHeight: CGFloat = 225
        let imageY = self.view.center.y - imageHeight
        let logoFrame = CGRect(x: self.view.frame.origin.x + marginsInset, y: imageY, width: self.view.frame.width - (marginsInset*2), height: imageHeight)
